...
module['exports'] = function spawnService (service, input, output) {
  ...
  if (typeof service.source === 'string') {
    service.code = service.source;
  }
  if (typeof service.code === "undefined") {
  ...
  return function spawnServiceMiddleware (input, output, next) {
    ...
    // If mschema is presenter, attempt to validate incoming input parameters before running the service
    if (service.schema) {
      // TOOD: should be async call to use mschema.conform property
      var validate = validateRequestParams(service, input, output);
      if (validate.valid === false) {
        return output.end(JSON.stringify(validate.errors, true, 2));
      }
    ...
    service.view = service.view || service.themeSource;
    service.presenter = service.presenterSource || service.presenter;

    var _presenter = service.presenter;
    ...
    // console.log('spawning'.blue, service);
    // don't want to evaluate service.presenter in the spawned binary ( was causing issues with argument gen for bash, maybe other langs ),
    // so we delete it here ( less data to pass to CLI )
    // TODO: instead of deleting properties, we should have a clone of the service object without these properties
    delete service.presenter;
    delete service.presenterSource;

    // determine if this service will require any stream transformations ( like the View-Presenter pattern )
    if ((typeof service.view !== "undefined" && service.view.length > 0 ) || typeof _presenter === "function") {
      viewPresenter({
        view: service.view,
    ...
    function _spawnService (input, output) {
      ...
      service.language = service.lang || service.language || "javascript";
      var log = config.log = config.log || console.log;
      ...
      if (typeof compileService[targetLanguage] === "function") {
        ...
        if (typeof compileCache[md5] === "undefined") {
          ...
          var compiled = compileService[targetLanguage](service.code);
      ...
      */
      ...
      var jsLangs = ["javascript", "babel", "coffee-script", "coffee", "es6", "es7"];
      if (jsLangs.indexOf(targetLanguage) === -1) {
        // Currently, only the `./bin/binaries/node target binary is capable of parsing multipart forms
        // This means for all non-node microservices, 
        // we currently will parse the incoming request here using the `parse-service-request` library
        // This will populate req.resource.params available, but not form fields,
        ...
        /*
        for (var p in input.resource.params) {
          __env.env[p] = input.resource.params[p];
        }
        /*
        psr(input, output, function (req, res, fields){
          console.log('got fields', fields, req.resource)
          for (var p in fields) {
            __env.params[p] = fields[p];
          }
        });
        */
        ...
        // Remark: For `node` based microservices, we have the `run-service` module spawned in the child process,
        // ( allowing better multipart support )
        preprocessCommandLineArguments();
        vm = spawn(targetBinary, binaryArgs /*, { stdio: [null, null, null, 'ipc'] } */);
        finish();
      }
