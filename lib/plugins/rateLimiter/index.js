...
RateLimiter.prototype.registerService = function (data, cb) {
  ...
  self.provider.hset(serviceReportKey, 'totalHits', 0, function (err,  totalHits) {
    cb(err);
  });
};
...
RateLimiter.prototype.removeService = function (data, cb) {
  ...
  cb = cb || function () {};
  var serviceReportKey = '/' + data.owner + '/' + data.name + '/report';
  self.provider.del(serviceReportKey, function (err,  totalHits) {
    cb(err);
  });
};
...
RateLimiter.prototype.middle = function rateLimitingMiddleware (config) {
  ...
  return function rateLimitingHandler (req, res, next) {
    ...
    provider.hget(userKey, 'monthlyHits', function (err,  monthlyHits) {
      provider.hget(serviceReportKey, 'totalHits', function (err,  totalHits) {
      if (totalHits === null || typeof totalHits === 'undefined') {
        // if no user report exists, assume this is an unknown service and immediately 410
        // 410 is used here to indicate the requesting server should no longer request this resource
        res.status(410);
        req.reduceCount = false;
        console.log('error: will not route unregistered service ' + serviceKey);
        return res.json({ 
          error: true, 
          message: 'Will not route unregistered service: ' + serviceKey,
          notice: 'The routing policy was changed on 4/12/2019. If this service exists and you are seeing this message, please re-save the service in order to register it.'
        });
      }
