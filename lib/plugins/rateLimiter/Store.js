...
var Store = function Store (provider) {
  ...
  self.services = {};
  self.files = {};
  console.log('Warning: Rate-Limiter is using default memory store. This is not suitable for production. Instead pass in a node-redis client.');
  return self;
};
...
Store.prototype.set = function (key, val, cb) {
  this.files[key] = this.files[key] || val;
  cb(null, true)
};
