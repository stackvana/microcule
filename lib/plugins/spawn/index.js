  ...
  */
  ...
  return function spawnServiceMiddleware (input, output, next) {
    ...
    if (typeof compileService[_service.language] === "function") {
      ...
      // TODO: move compile handler to higher scope? probably not too terrible of performance hit right now
      var _compile = compile(_service);
    ...
    function _spawnService (input, output) {
      ...
      */
      ...
      // console.log('spawning', binaryArgs, process.cwd(), targetBinary, binaryArgs)

      vm = spawn(targetBinary, binaryArgs, { stdio: ['pipe', 'pipe', 'pipe', 'pipe'] });
