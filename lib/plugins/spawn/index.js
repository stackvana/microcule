      ...
      function finish () {
        ...
        function endResponse () {
          ...
          if (status.serviceEnded) {
            ...
            //
            // Important: For services without integrated response.end() API ( everything but JavaScript ),
            // spawn is always going to be asssume there are no additional middlewares to render and end output
            if (middlewareEnabledLanguages.indexOf(_service.language) === -1) {
              // End the output for non-JS services inside spawn ( we could change this behavior )
              output.end();
            } else {
              next();
            }
