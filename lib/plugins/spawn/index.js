...
module['exports'] = function spawnService (service) {
  ...
  _service.originalCodeFilePath = service.originalCodeFilePath;
  ...
  */
  ...
  return function spawnServiceMiddleware (input, output, next) {
    ...
    if (typeof compileService[_service.language] === "function") {
       console.log('found compile service for '.yellow, _service.language);
      ...
      _compile(input, output, function (err, _build) {
        ...
        if (_build.exitCode !== 0 && _build.foundCompiledCache !== true) {
         ...
         console.log('Compiler result: '.yellow);
         //console.log('### spawnServiceMiddleware._compile() ###')
         console.dir(_build);
        ...
        if (_service.language === 'dotnet'){
          _service.bin = 'dotnet';
          _service.buildDir = _build.buildDir + '/' + _build.dotnetCsproj; //needed in commandlineArgs generator
