  ...
  return function spawnServiceMiddleware (input, output, next) {
    ...
    function _afterParseCheck () {
      ...
      function _spawnService (input, output) {
        ...
        // legacy support for older versions when Node didn't supprot native async / await
        if (targetLanguage === "es6" || targetLanguage === "es7" || targetLanguage === 'babel') {
          targetLanguage = "javacsript";
        }
