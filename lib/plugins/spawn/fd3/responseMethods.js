...
methods.status = function (message, res) {
  if (typeof message.payload.value === "number" && message.payload.value >= 100 && message.payload.value < 1000) {
    res.status(message.payload.value);
  }
};
