...
module.exports = function compile (config) {
  ...
  config.releaseDir = config.releaseDir || config.service.home + '/release';
  ...
  var _service = {};
  _service.home = config.service.home;
  ...
  return function compileMiddleware (req, res, next) {
    ...
    var absoluteBinLocation, relatitveBinLocation;

    absoluteBinLocation = _service.releaseDir + '/' + sha;
    relatitveBinLocation = '/release/' + sha;

    // java requires a fixed class name, so we must create a unique directory instead
    if (_service.language === 'java') {
      absoluteBinLocation = _service.buildDir + '/' + sha + '/' + 'hook.class';
    }
    ...
    fs.stat(absoluteBinLocation, function (err, _stat) {
      ...
      if (err) {
        ...
        var result = {
          bin: _service.home + relatitveBinLocation,
