...
module.exports = function compile (config) {
  ...
  return function compileMiddleware (req, res, next) {
    ...
    fs.stat(_service.releaseDir + '/' + sha, function (err, _stat) {
      console.log('back from stat', err, _stat);
      if (err) {
        // could not find the file, attempt to compile it
        console.log('could not find file, so lets compile it');
        ...
        console.log('using compiled version of service', _service.releaseDir + '/' + sha);
        var result = {
          bin: _service.releaseDir + '/' + sha,
          compiledFresh: false,
