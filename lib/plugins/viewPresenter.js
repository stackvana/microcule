...
module.exports = function viewPresenterMiddleware (config) {
  ...
  return function viewPresenterHandler (req, res, next) {
    ...
    // Note: In order to modify the response stream after this middleware is done,
    // we must monkey patch response methods to allow for buffering of View-Presenter output into memory
    // This allows us to use the later calculated value of the hook output inside of the View-Presenter
    var write = res.write, 
        end = res.end;
    ...
    */
      ...
      try { // this will catch user run-time errors in the presenter
        _view.present({
          ...
          try {
            ...
            end('Failed to call res.end ' + err.message);
          }
        });
      } catch (err) {
        completed = true;
        ...
        end('Error in Presenter code: ' + err.message + '\n\n' + err.stack);
      }
    ...
    // once the View-Presenter is monkey patched onto the response, continue
    // Note: The View-Presenter won't be run until future res.write / res.end calls are made
    next();
  };
